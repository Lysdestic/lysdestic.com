...
- [x] **External links hardening** — use `rel="noopener noreferrer"` anywhere `target="_blank"` is used
- [x] **Install `@astrojs/check` and add `npm run check`** — enable typed Astro checks in regular workflow
...
- [x] **`about.astro` interest grid** — replaced table markup with semantic list cards and CSS grid for cleaner responsive behaviour
...
- [x] **Run Prettier cleanup pass** — completed with `npm run format`; parser issue in `VideoPlayer.astro` was fixed so formatting can run cleanly
